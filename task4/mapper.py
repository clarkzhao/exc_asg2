...
for line in sys.stdin:
    ...
    if post_typeId.split('=')[-1] == '"1"':
        for token in tokens:
            if re.search("AcceptedAnswerId",token):
                acc_anw_id = token
                acc_anw_id_num = int(re.findall(r'\d+',acc_anw_id)[0])
                break
        #<ANS, accepted answer ID> is the output, 'ANS' is a notation for accepted answer
        print ("ANS {0}".format(acc_anw_id_num))
    #When it's a answer, print the owner User Id and answer Id
    else:
        Id = tokens[1]
